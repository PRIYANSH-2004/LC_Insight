...
const CompareUserCard: React.FC<UserDataProps> = (props) => {
        ...
        <div className="bg-white shadow rounded-lg p-6 ">
          <div className="flex items-center gap-4">
            ...
            <div className="min-w-0">
              <h2
                className="text-2xl font-bold text-black truncate max-w-xs"
              ...
              title={userData?.data?.matchedUser?.username ?? "null"}>
                @{userData?.data?.matchedUser?.username ?? "null"}
              </p>
              <p className="text-black">
                {userData?.data?.matchedUser?.profile?.countryName ?? "null"} •{" "}
                {userData?.data?.matchedUser?.profile?.school ?? "null"}
              </p>
              <p className="text-black">
                Global Ranking: #
                {userData?.data?.matchedUser?.profile?.ranking ?? "null"}
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            <div className="p-4 bg-gray-50 rounded">
              ...
              <p className="text-xl font-bold text-black">
                {userData?.data?.userContestRanking?.rating?.toFixed(2) ??
                  "null"}
              </p>
            </div>
            <div className="p-4 bg-gray-50 rounded">
              ...
              <p className="text-xl font-bold text-black">
                #{userData?.data?.userContestRanking?.globalRanking ?? "null"}
              </p>
            </div>
            <div className="p-4 bg-gray-50 rounded">
              <p className="text-gray-600">Top Percentage</p>
              <p className="text-xl font-bold text-black">
                {userData?.data?.userContestRanking?.topPercentage?.toFixed(
                  1
                ) ?? "null"}
                %
              </p>
            ...
            <div className="p-4 bg-gray-50 rounded">
              ...
              <p className="text-xl font-bold text-black">
                {userData?.data?.userContestRanking?.attendedContestsCount ??
                  "null"}
              </p>
            </div>
          </div>
